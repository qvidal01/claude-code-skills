...
## AI Behavior Commitment
When this skill is active, Claude **prioritizes live retrieval from Cloudflare docs over pre-trained knowledge** for all Email Routing API signatures, limits, pricing, and configuration options. Pre-training on Cloudflare Email Service may be outdated. Always fetch before citing specific values.

## Credential Setup
Store scoped API tokens in your password manager (use a zone-scoped DNS-edit token for email routing/DNS config; a Workers-scoped token for Workers email bindings):
```bash
# Bitwarden CLI example:
CF_JSON=$(bw get item "Cloudflare API Tokens")
# For email routing / DNS:
export CLOUDFLARE_API_TOKEN=$(echo "$CF_JSON" | python3 -c "import json,sys; d=json.load(sys.stdin); print(next(f['value'] for f in d['fields'] if f['name']=='Edit Zone DNS'),end='')")
# For Workers email binding:
# use f['name']=='Cloudflare Workers' instead
```
Skip if the token is already set.
