...
description: "Cloudflare Workers CLI for deploying, developing, and managing Workers, KV, R2, D1, Vectorize, Hyperdrive, Workers AI, Containers, Queues, Workflows, Pipelines, and Secrets Store. Load before running wrangler commands to ensure correct syntax and best practices. Biases towards retrieval from Cloudflare docs over pre-trained knowledge. TRIGGER WHEN: 'deploy a Worker', 'run wrangler dev', 'configure wrangler.toml', 'publish to Cloudflare', 'wrangler secret', 'wrangler tail', 'wrangler d1 migrations'. DO NOT USE WHEN: working with non-Wrangler Cloudflare tasks (use /cloudflare)."
---

# Wrangler CLI

## When To Use
- "deploy a Cloudflare Worker / Pages project"
- "run `wrangler dev` / local dev server"
- "configure `wrangler.toml`"
- "manage secrets / env vars via `wrangler secret`"
- "run D1 migrations with wrangler"
- "tail production logs with `wrangler tail`"

## When NOT To Use
- General Cloudflare platform questions (use `/cloudflare`)
- Non-Workers deployments

## Credential Setup (Bitwarden)
Store a Workers-scoped API token in your password manager and export it for the session:
```bash
# Bitwarden CLI example (field: `Cloudflare Workers`):
...
```
`CLOUDFLARE_ACCOUNT_ID` can be found in the Cloudflare dashboard URL.
