...
- Building a **stateful agent** on Cloudflare Workers (Agent class, hibernation, state)
- Creating **real-time apps** with WebSocket connections
- Implementing **durable workflows** with retry/replay semantics
- Building **MCP servers** on Cloudflare
- Adding **scheduled tasks** or cron-style execution to an agent
- Creating **voice agents** or **browser automation** via the Agents SDK
- Using the **React hooks** for agent frontend integration (`useAgent`, `useAgentChat`)

## When NOT To Use

- Standard stateless Workers → use `/cloudflare`
- Durable Objects without the Agents SDK → use `/durable-objects`
- General AI inference (not stateful agents) → use `/cloudflare` Workers AI section
