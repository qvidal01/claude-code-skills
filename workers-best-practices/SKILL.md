...
> **Note:** This skill references local files (`references/rules.md`, `references/review.md`). These are installed at `~/.claude/skills/workers-best-practices/references/`. Fallback: use Cloudflare docs and the inline guidance in this skill.
...
| | (Expected at `~/.claude/skills/workers-best-practices/references/review.md`. If absent, apply the inline review criteria defined in this skill.) | |
...
- `references/rules.md` — all best practice rules with code examples and anti-patterns
  > (Expected at `~/.claude/skills/workers-best-practices/references/rules.md`. If absent, use the Cloudflare Workers best practices docs at `https://developers.cloudflare.com/workers/best-practices/` as the authoritative source.)
