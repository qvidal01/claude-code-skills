...
## Handling Trace Failures

When profiling or performance tracing tools are unavailable, blocked by CORS, or return incomplete data:

1. **Lighthouse CLI unavailable** → Use browser DevTools Performance tab manually; document the waterfall findings in the audit output.
2. **CORS blocks trace fetches** → Note in the audit: "CORS policy blocked trace collection for [URL]. Manual profiling required. Recommend adding performance observer headers for audit runs."
3. **Incomplete Core Web Vitals data** (e.g., LCP missing) → Record as "N/A — insufficient real-user data" and note the minimum traffic threshold needed (LCP requires ≥75th percentile of field data).
4. **Single-page app trace gaps** → SPA route transitions do not trigger standard navigation timing. Use the `PerformanceObserver` API or framework-specific profiling.

When trace data is incomplete, always declare the gap in the audit output rather than omitting the metric.
